...
MIN_MACOS_VERSION = "13.5" # For macOS 14 and below
MIN_MACOS_VERSION_EXPERT = "12.3"

# Special handling for macOS 15+
MAC_OS_15_SUPPORTED = True
MIN_MACOS_15_VERSION = "15.0"
...
DEVICES = {
    ...
    "j514sap":  Device("15.0", False),  # MacBook Pro (14-inch, M3 Pro, 2023)
}
...
class InstallerMain:
    ...
    def main(self):
        ...
        # Check if using macOS 15+
        if MAC_OS_15_SUPPORTED and self.sysinfo.macos_ver.startswith("15."):
            # Special version check for macOS 15+
            if split_ver(self.sysinfo.macos_ver) < split_ver(MIN_MACOS_15_VERSION):
                p_error("Your macOS 15 version is too old.")
                p_error(f"Please upgrade to macOS {MIN_MACOS_15_VERSION} or newer.")
                sys.exit(1)
            # macOS 15.x passes the version check
            logging.info(f"macOS 15.x detected (version {self.sysinfo.macos_ver}), accepting as compatible")
        else:
            # Standard version check for macOS 14 and earlier
            if self.expert:
